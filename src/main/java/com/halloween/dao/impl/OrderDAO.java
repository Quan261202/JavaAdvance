...
public class OrderDAO extends AbstractDAO<OrderModel> implements IOrderDAO{
    ...
    public List<RecentOrder> getRecentOrder() {
		...
		StringBuilder sql = new StringBuilder("select cdt.cusName, SUM(price) as totalPrice, o.status from orders o inner join customerDetail cdt on o.customerID = cdt.cusID \r\n" + "	inner join orderItem od on od.orderID = o.id where o.status > 0 group by o.customerID, o.id");
		...
		try {
			...
			while (resultSet.next())
			{
				...
				recentOrders.add(new RecentOrder(resultSet.getString(1), resultSet.getDouble(2), status, colorSta));
			}
